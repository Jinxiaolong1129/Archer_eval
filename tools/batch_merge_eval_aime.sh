#!/bin/bash
#
# 批量模型合并与 AIME 评估脚本 - Part 1 (5 GPU)
...
mkdir -p ${LOG_DIR}
MAIN_LOG=${LOG_DIR}/batch_merge_eval_part1_$(date +%Y%m%d_%H%M%S).log

# ============ 实验列表 Part 1 (5个实验) ============
EXPERIMENTS=(
    "Archer-TokenEntropy-Qwen2.5-1.5B-2k-8k-batch64-no-kl-simple"
    "Archer-ProbDisparity-Qwen2.5-1.5B-2k-8k-batch64-no-kl-simple"
    "Archer-ProbDisparity-Qwen2.5-1.5B-2k-8k-batch64-kl005"
    "Archer-ProbDisparity-Qwen2.5-1.5B-2k-8k-batch64-no-kl-n8"
    "Archer-Intuitor-Qwen2.5-1.5B-2k-8k-batch64-no-kl-temp1.2"
)
...
main() {
    log "============================================================"
    log "批量模型合并与 AIME 评估脚本 - Part 1 (5 GPU)"
    log "============================================================"
    log "实验数量: ${#EXPERIMENTS[@]}"
    log "Checkpoint steps: ${STEPS[*]}"
    log "GPU: 3,4,5,6,7 (${n_gpus} GPUs)"
