...
# 评估参数 (Code generation task)
n_samples=8          # 每个问题生成 8 个回答用于计算 pass@K
temperature=0.6      # 代码生成温度
top_p=0.95
max_prompt_length=$((1024 * 4))      # 4K prompt (代码题目可能较长)
max_response_length=$((1024 * 8))    # 8K response (先测试速度)
batch_size=10        # 小批次，方便观察进度

# 输出路径 (包含响应长度标识)
response_len_k=$((max_response_length / 1024))
output_dir=${base_dir}/output/eval_results/${dataset}_${response_len_k}k
mkdir -p ${output_dir}
...
echo "============================================================"
echo "LiveCodeBench v5 Evaluation"
echo "============================================================"
